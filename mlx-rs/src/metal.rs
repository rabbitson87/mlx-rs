...
/// the (`lumen_turboquant_qk_inline` + `softmax` +
/// `lumen_turboquant_sv_inline`) 3-dispatch chain at decode (T=1).
///
/// Shape:
///   - `q`        : bf16  `[B, H,    T=1, D]`
///   - `k_codes`  : uint8 `[B, H_kv, N,   D]`
///   - `k_sigma`  : f32   `[B, H_kv, N]`        (squeeze trailing-1)
///   - `v_codes`  : uint8 `[B, H_kv, N,   D]`
///   - `v_sigma`  : f32   `[B, H_kv, N]`
///   - `centroids`: f32   `[n_levels]`          (Lloyd-Max LUT; n_levels ≤ 16)
///   - `scale`    : f32 host-side scalar — attention scale (= 1/sqrt(D)
///                  * any external softcap factor)
///   - returns    : bf16  `[B, H,    T=1, D]`
///
/// Constraints: `T == 1`, `D ∈ {256, 512}`, `n_levels ≤ 16`, `H % H_kv == 0`.
#[track_caller]
pub fn lumen_turboquant_fused_attn(
    q: &Array,
    k_codes: &Array,
    k_sigma: &Array,
    v_codes: &Array,
    ...
    Array::try_from_op(|res| unsafe {
        mlx_sys::mlx_lumen_turboquant_fused_attn(
            res,
            q.as_ptr(),
            k_codes.as_ptr(),
            k_sigma.as_ptr(),
